...
$response | ConvertTo-Json | Out-File "_siteroot/versions.json"

# Only update web.config if we are the latest version
if ($response.versions[0] -eq $deploymentProfile)
{
	(Get-Content -path "web.config" -Raw) -Replace '%deployment_version%', $version | Set-Content -Path "_siteroot\web.config"
}
